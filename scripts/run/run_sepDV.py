...
dataset = sys.argv[1]
save_folder = root+'output/CombGen/31Jul2020_1x1train_32test_8x8regimes_sepDV_wo_minmax_400epochs_wl11/'+dataset
setup = sys.argv[2]
exp_name = sys.argv[3]
ood=True
    ...
    for emb_dims in [40]:
        for p in [0.0]:
            for h in [5]:#,25,30,35,45,55,80,105]:
                if ood:
                    base_cmd="python train_sepDV.py --path "+root+"/data/narma/combgen/4/test_ood/ --dataset " + dataset + " --save-folder "+save_folder+" --batch-size "+str(bsz)+" --epochs "+ str(epochs) +" --dec_epochs "+str(dec_epochs) + " --embedding_dim "+ str(emb_dims) + " --nodes "+ str(gnn_nodes) +" --shuffle --dropout " + str(p)+" --length "+str(length)+" --window_size "+str(window_size)+ " --horizon " + str(h) + " --shift "+str(shift) +" --num_objects "+str(num_objects)+" --num_cont "+str(num_control)+" --stride "+str(stride) + " --learning_rate 0.001 --full --ood "
                else:
                    base_cmd="python train_sepDV.py --path "+root+"/data/narma/combgen/4/train_test_iid/ --dataset " + dataset + " --save-folder "+save_folder+" --batch-size "+str(bsz)+" --epochs "+ str(epochs) +" --dec_epochs "+str(dec_epochs) + " --embedding_dim "+ str(emb_dims) + " --nodes "+ str(gnn_nodes) +" --shuffle --dropout " + str(p)+" --length "+str(length)+" --window_size "+str(window_size)+ " --horizon " + str(h) + " --shift "+str(shift) +" --num_objects "+str(num_objects)+" --num_cont "+str(num_control)+" --stride "+str(stride) + " --learning_rate 0.001 --full "
                if exp_name=='B':
