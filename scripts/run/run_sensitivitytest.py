...
dataset = sys.argv[1]
save_folder = root+'output/CombGen/3Aug2020_1x1train_32test_8x8regimes_wo_minmax_400epochs_wl11/'+dataset
setup = sys.argv[2]
exp_name = sys.argv[3]
ood=True
    ...
    for emb_dims in [40]:
        for p in [0.0]:
            for h in [5]:#,25,30,35,45,55,80,105]:
                ...
                if exp_name=='B':
                    os.system(base_cmd + "--name B --baseline --noise --pert 0")                    
                    os.system(base_cmd + "--name B --baseline --noise --pert 1")
                elif exp_name=='B+GL':
                    ...
                    os.system(base_cmd + "--name O+HD --hierarchical_ls --sepCTRL --layer_gl --gl "+str(gl)+" --hard_decoder --noise --pert 0")
                    os.system(base_cmd + "--name O+HD --hierarchical_ls --sepCTRL --layer_gl --gl "+str(gl)+" --hard_decoder --noise --pert 1")
                elif exp_name=='O-SC':
                    ...
                    os.system(base_cmd + "--name O-GL --hierarchical_ls --sepCTRL --noise --pert 0")
                    os.system(base_cmd + "--name O-GL --hierarchical_ls --sepCTRL --noise --pert 1")
                elif exp_name=='O-HN-GL':
                    os.system(base_cmd + "--name O-HN-GL --baseline --sepCTRL --noise --pert 0")
                    os.system(base_cmd + "--name O-HN-GL --baseline --sepCTRL --noise --pert 1")
                elif exp_name=='O+HD-GL':
                    os.system(base_cmd + "--name O+HD-GL --hierarchical_ls --sepCTRL --hard_decoder --noise --pert 0")
                    os.system(base_cmd + "--name O+HD-GL --hierarchical_ls --sepCTRL --hard_decoder --noise --pert 1")
