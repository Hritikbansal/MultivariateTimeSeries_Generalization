...
import mts_model_sepDV as mts_model
import sys
import helper
...
used_params='-'.join([args.dataset, str(args.batch_size), str(args.embedding_dim),str(args.full),str(args.learning_rate),str(args.baseline), str(args.sepCTRL),str(args.layer_gl), str(args.gl),str(args.hierarchical_ls),str(args.soft_decoder_gl),str(args.decoder_gl),str(args.hard_decoder)])
...
model = mts_model.MVTS(
	...
	hard_decoder=args.hard_decoder).to(device)
		...
		for batch_idx, data_batch in enumerate(loader):
			...
			for i in range(action.shape[2]):
				state_encoding, cont_encoding, action_encoding = model.getEncodings(torch.cat([statePast[:,:,i:],nextState[:,:,:i]],dim=-1), 
						torch.cat([contPast[:,:,i:],action[:,:,max(0,i-args.window_size):i]],dim=-1), action[:,:,i].unsqueeze(2))
...
if args.noise:
	...
	for r in ratio:
		...
		if r==0:		
			...
			evaluate_forecasting(model, test_loader, args, isTest=True)
		else:
			# evaluate_forecasting(model, utils.addnoise_ratio(validation_loader, r, args.pert),args)
			print("Testing noise on test set!!!")
			evaluate_forecasting(model, utils.addnoise_ratio(test_loader, r, args.pert),args, isTest=True)
